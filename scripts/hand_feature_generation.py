...
def loadRobot(env):
	env.Load('/home/yiherngong/Documents/openrave/src/robots/barretthand.robot.xml')
	return env.GetRobots()[0]

def loadObject(env, obj_name):
    env.Load('./' + obj_name + '.ply', {'scalegeometry':'0.001'})
...
#	print len(verts)
#	return verts
def getObjectVerts(filename):
	b = numpy.loadtxt(filename, dtype=float)
	b = list(numpy.reshape(b, (-1,3)))
	return b	
...
def centerItem(robot, item, bounds, bounding_item):
